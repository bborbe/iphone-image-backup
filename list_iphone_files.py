    ...
    def print_files(self, files: list, show_details: bool = False):
        ...
        for file_info in files:
            if show_details:
                ...
                print(
                    f"   Size: {size_str:>10} | Modified: {modified_str} | Ext: {file_info['extension']}"
                )
                print()
            else:
                # Simple view
...
def main():
    ...
    parser.add_argument(
        "-d", "--directory", default="/", help="Directory to scan (default: / for root)"
    )
    parser.add_argument(
        "-r",
        "--recursive",
    ...
    parser.add_argument("--no-recursive", action="store_true", help="Scan only current directory")
    parser.add_argument("--details", action="store_true", help="Show detailed file information")
    parser.add_argument("--photos-only", action="store_true", help="Show only photo/video files")
    ...
    files = lister.list_directory(
        args.directory, recursive=recursive, show_details=args.details, photos_only=args.photos_only
    )
